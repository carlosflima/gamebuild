package com.carlosflima.gamebuild.data

import com.carlosflima.gamebuild.domain.Game
import com.carlosflima.gamebuild.domain.GameCharacter

internal object EndfieldCharacterCatalog {
    val characters: List<GameCharacter> = listOf(
        GameCharacter(
            id = "endfield-endministrator",
            name = "Endministrator",
            role = "Guard · Physical",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-perlica",
            name = "Perlica",
            role = "Caster · Electric",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-chen-qianyu",
            name = "Chen Qianyu",
            role = "Guard · Physical",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-wulfgard",
            name = "Wulfgard",
            role = "Caster · Heat",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-yvonne",
            name = "Yvonne",
            role = "Striker · Cryo",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-typhoeus",
            name = "Typhoeus",
            role = "Striker · Nature",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-antal",
            name = "Antal",
            role = "Supporter · Electric",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-akekuri",
            name = "Akekuri",
            role = "Vanguard · Heat",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-avywenna",
            name = "Avywenna",
            role = "Striker · Electric",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-catcher",
            name = "Catcher",
            role = "Defender · Physical",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-alesh",
            name = "Alesh",
            role = "Vanguard · Cryo",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-xaihi",
            name = "Xaihi",
            role = "Supporter · Cryo",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-snowshine",
            name = "Snowshine",
            role = "Defender · Cryo",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-arclight",
            name = "Arclight",
            role = "Vanguard · Electric",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-estella",
            name = "Estella",
            role = "Guard · Cryo",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-fluorite",
            name = "Fluorite",
            role = "Caster · Nature",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-ardelia",
            name = "Ardelia",
            role = "Supporter · Nature",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-ember",
            name = "Ember",
            role = "Defender · Heat",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-lifeng",
            name = "Lifeng",
            role = "Guard · Physical",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-pogranichnik",
            name = "Pogranichnik",
            role = "Vanguard · Physical",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-last-rite",
            name = "Last Rite",
            role = "Striker · Cryo",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-gilberta",
            name = "Gilberta",
            role = "Supporter · Nature",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-liino",
            name = "Liino",
            role = "Supporter · Electric",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-tangtang",
            name = "Tangtang",
            role = "Caster · Cryo",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-laevatain",
            name = "Laevatain",
            role = "Striker · Heat",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-zhuang-fangyi",
            name = "Zhuang Fangyi",
            role = "Striker · Electric",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-da-pan",
            name = "Da Pan",
            role = "Striker · Physical",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-mi-fu",
            name = "Mi Fu",
            role = "Guard · Physical",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-camille",
            name = "Camille",
            role = "Vanguard · Heat",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-rossi",
            name = "Rossi",
            role = "Guard · Physical",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-arcane",
            name = "Arcane",
            role = "Caster · Nature",
            game = Game.ENDFIELD
        ),
        GameCharacter(
            id = "endfield-purrchena",
            name = "Purrchena",
            role = "Defender · Physical",
            game = Game.ENDFIELD
        )
    )
}
