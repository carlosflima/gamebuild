...
class GameBuildViewModelTest {
    ...
    @Test
    fun defaultRepositoryLoadsEndfieldRosterAndXaihiBuild() {
        ...
        assertEquals(
            listOf("Endministrator", "Perlica", "Chen Qianyu", "Wulfgard", "Yvonne", "Typhoeus", "Antal", "Akekuri", "Avywenna", "Catcher", "Alesh", "Xaihi"),
            rosterState.characters.map { it.name }
        )
        assertEquals(setOf(Game.ENDFIELD), rosterState.characters.map { it.game }.toSet())

        val xaihi = rosterState.characters.first { it.name == "Xaihi" }
        viewModel.selectCharacter(xaihi)

        val buildState = viewModel.uiState.value
        assertEquals("Xaihi", buildState.selectedCharacter?.name)
        assertEquals(
            listOf("endfield-xaihi-f2p-starter-2026-09"),
