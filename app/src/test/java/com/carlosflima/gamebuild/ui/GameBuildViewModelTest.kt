...
class GameBuildViewModelTest {
    ...
    @Test
    fun defaultRepositoryLoadsEndfieldRosterAndLaevatainBuild() {
        ...
        assertEquals(
            listOf("Endministrator", "Perlica", "Chen Qianyu", "Wulfgard", "Yvonne", "Typhoeus", "Antal", "Akekuri", "Avywenna", "Catcher", "Alesh", "Xaihi", "Snowshine", "Arclight", "Estella", "Fluorite", "Ardelia", "Ember", "Lifeng", "Pogranichnik", "Last Rite", "Gilberta", "Liino", "Tangtang", "Laevatain"),
            rosterState.characters.map { it.name }
        )
        assertEquals(setOf(Game.ENDFIELD), rosterState.characters.map { it.game }.toSet())

        val laevatain = rosterState.characters.first { it.name == "Laevatain" }
        viewModel.selectCharacter(laevatain)

        val buildState = viewModel.uiState.value
        assertEquals("Laevatain", buildState.selectedCharacter?.name)
        assertEquals(
            listOf("endfield-laevatain-f2p-starter-2026-09"),
