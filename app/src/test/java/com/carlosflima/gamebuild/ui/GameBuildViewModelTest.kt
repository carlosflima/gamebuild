...
class GameBuildViewModelTest {
    ...
    @Test
    fun defaultRepositoryLoadsEndfieldRosterAndAleshBuild() {
        ...
        val alesh = rosterState.characters.first { it.name == "Alesh" }
        viewModel.selectCharacter(alesh)

        val buildState = viewModel.uiState.value
        assertEquals("Alesh", buildState.selectedCharacter?.name)
        assertEquals(
            listOf("endfield-alesh-f2p-starter-2026-09"),
