...
class GameBuildViewModelTest {
    ...
    @Test
    fun defaultRepositoryLoadsEndfieldRosterAndSnowshineBuild() {
        ...
        assertEquals(
            listOf("Endministrator", "Perlica", "Chen Qianyu", "Wulfgard", "Yvonne", "Typhoeus", "Antal", "Akekuri", "Avywenna", "Catcher", "Alesh", "Xaihi", "Snowshine"),
            rosterState.characters.map { it.name }
        )
        assertEquals(setOf(Game.ENDFIELD), rosterState.characters.map { it.game }.toSet())

        val snowshine = rosterState.characters.first { it.name == "Snowshine" }
        viewModel.selectCharacter(snowshine)

        val buildState = viewModel.uiState.value
        assertEquals("Snowshine", buildState.selectedCharacter?.name)
        assertEquals(
            listOf("endfield-snowshine-f2p-starter-2026-09"),
